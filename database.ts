...
export const fetchGloballyUsedUnlockOrderIds = async (): Promise<string[]> => {
    try {
        ...
        const ids: string[] = [];
        // Regex per trovare l'ID dell'ordine target dopo la stringa "all'ordine "
        data?.forEach((m: any) => {
        ...
        return [...new Set(ids)]; // Ritorna ID univoci
    } catch (e) {
...
export const updateSale = (id: string, updates: Partial<Sale>) => {
    const mapped: any = {};
    // Elenco completo dei campi inclusi quelli per la cronologia contatti e varianti
    const fields = [
...
export const createNotification = async (n: any) => {
    ...
    if (res.error) {
        ...
        alert(`ERRORE DATABASE: ${res.error.message}\nCodice: ${res.error.code}\nDettagli: ${res.error.details}`);
        throw res.error;
    }
    return res;
};
...
export const createTicket = async (t: any) => supabase.from('tickets').insert({ ...t, status: 'Aperto' });
export const addTicketReply = async (tId: string, r: any) => supabase.from('ticket_replies').insert({ ...r, ticket_id: tId });
